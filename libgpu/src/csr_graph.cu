...
unsigned CSRGraph::allocOnDevice(bool no_edge_data) {
  ...
  check_cuda(cudaMallocManaged((void **) &edge_dst, nedges * sizeof(index_type)));
  check_cuda(cudaMallocManaged((void **) &row_start, (nnodes+1) * sizeof(index_type)));

  if (!no_edge_data) check_cuda(cudaMallocManaged((void **) &edge_data, nedges * sizeof(edge_data_type)));
  check_cuda(cudaMallocManaged((void **) &node_data, nnodes * sizeof(node_data_type)));
...
void CSRGraphTex::copy_to_gpu(struct CSRGraphTex &copygraph) {
  ...
  memcpy(copygraph.edge_dst, edge_dst, nedges * sizeof(index_type));
  if (edge_data != NULL) memcpy(copygraph.edge_data, edge_data, nedges * sizeof(edge_data_type));
  memcpy(copygraph.node_data, node_data, nnodes * sizeof(node_data_type));

  memcpy(copygraph.row_start, row_start, (nnodes+1) * sizeof(index_type));
}
